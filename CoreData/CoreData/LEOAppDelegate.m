...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [self CreateUserWithName:@"Leo" age:28];
    ...
    if([users count]>0){
        for (User *user in users){
            NSLog(@"user name: %@ age:%d",user.name,[user.age integerValue]);
        }
    }
...
-(BOOL) CreateUserWithName:(NSString *)name
                       age:(NSInteger)age{
    BOOL result=NO;
    User *user=[NSEntityDescription insertNewObjectForEntityForName:@"User" inManagedObjectContext:self.managedObjectContext];
    
    if(user !=nil){
        [user setAge:[NSNumber numberWithUnsignedInteger:age]];
        [user setName:name];
